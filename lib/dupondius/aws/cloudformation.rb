...
module Dupondius; module Aws; module CloudFormation
  ...
  class Dashboard < Stack
    ...
    def initialize project_name, tech_stack, aws_region, key_name, parameters
      @project_name, @tech_stack, @aws_region, @key_name, @parameters = project_name, tech_stack, aws_region, key_name, parameters
    end
    ...
    def load_template
      ...
      user_data.insert((user_data.size) -4, "curl -L https://s3.amazonaws.com/dupondius_config/install-dashboard | bash -s #{@tech_stack.parameterize} #{@aws_region} #{@key_name}\n")
